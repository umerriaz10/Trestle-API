...
```bash
Trestle-API$ sam build --use-container
```
...
```bash
Trestle-API$ sam logs -n StockCheckerFunction --stack-name "trestle-api" --tail
```
...
```bash
Trestle-API$ pip install -r tests/requirements.txt --user
# unit test
Trestle-API$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
Trestle-API$ AWS_SAM_STACK_NAME="trestle-api" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "trestle-api"
```
